...
Player::Player()
	:m_EnginePower(1.0f), m_Gravity(0.8f)
{
	//Smoke
...
{
	m_ShipTexture = Independent::Texture2D::Create("assets/textures/Ship.png");
}

void Player::OnUpdate(Independent::Timestep ts)
{
	m_Time += ts;

	if (Independent::Input::IsKeyPressed(IDPD_KEY_SPACE))
	{
...
void Player::OnRender()
{
	...
	Independent::Renderer2D::DrawRotatedQuad({m_Position.x, m_Position.y, 0.5f}, { 0.77f, 1.23f }, glm::radians(GetRotation()), m_ShipTexture);   
}
